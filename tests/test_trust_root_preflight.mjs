...
async function main() {
  ...
  await test("production mode without a ledger signing key fails startup preflight", async () => {
    const dir = mkdtempSync(join(tmpdir(), "mnde-tr-"));
    try {
      const env = await writeProductionCustody(dir);
      delete env.MNDE_LEDGER_SIGNING_KEY;
      delete env.MNDE_LEDGER_KEY_ID;
      const r = await assertTrustRoot(env, { repoRoot });
      assert.equal(r.ok, false);
      assert.equal(r.reason_code, "ERR_TRUST_ROOT_LEDGER_SIGNER");
      assert.match(r.detail, /ledger signer self-test failed/);
    } finally { rmSync(dir, { recursive: true, force: true }); }
  });
  ...
  await test("sidecar STARTS in production with valid custody + caller auth + enforced policy", async () => {
    ...
    try {
      const env = {
        ...
        ...(await productionEnforcementExtras(dir)),
        MNDE_BIND_PORT: "8792"
      };
