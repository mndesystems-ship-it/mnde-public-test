...
async function main() {
  ...
  await test("sidecar REFUSES to start in production without caller auth", async () => {
    ...
    try {
      const env = { ...writeProductionCustody(dir), MNDE_BIND_PORT: "8810", MNDE_RECEIPT_LOG: join(dir, "r.jsonl") };
  ...
  await test("sidecar REFUSES to start in production with auth but legacy engine", async () => {
    ...
    let started = null, threw = false, message = "";
    try {
      const env = { ...writeProductionCustody(dir), ...validAuthEnv(), MNDE_BIND_PORT: "8811", MNDE_RECEIPT_LOG: join(dir, "r.jsonl") };
