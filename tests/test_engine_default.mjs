...
async function main() {
  ...
  await test("legacy ecs.receipt.v2 receipt still verifies (verify-only, kind pipeline)", async () => {
    ...
    const v = await verifyAnyReceiptObject(legacy);
  ...
  await test("zero-config default: policy-engine + default-deny REFUSE with a clean receipt", async () => {
    ...
    try {
      ...
      const verdict = await verifyAnyReceiptObject(body.receipt);
