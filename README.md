...
Optional executor identity produces explicit `executor_and_authority` live receipts with no authority-only fallback. Without executor configuration, live receipts remain `authority_only`. See [executor-bound live receipts](docs/executor-bound-receipts.md) for configuration, verification, private-key path restrictions, and exact security claims.
